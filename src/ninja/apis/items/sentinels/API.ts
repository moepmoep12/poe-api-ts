...
import { SentinelOverview } from "./ItemOverview";
...
): Promise<SentinelOverview> => {
  return await getOverviewGeneric(league, ItemOption.Sentinel, language, SentinelOverview);
};
...
export const getHistory = async (league: string, id: number): Promise<HistoryPoint[]> => {
  return await getHistoryGeneric(league, ItemOption.Sentinel, id);
};
