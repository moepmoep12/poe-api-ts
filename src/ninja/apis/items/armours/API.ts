...
import { UniqueArmourOverview } from "./ItemOverview";
...
): Promise<UniqueArmourOverview> => {
  return await getOverviewGeneric(league, ItemOption.UniqueArmour, language, UniqueArmourOverview);
};
...
export const getHistory = async (league: string, id: number): Promise<HistoryPoint[]> => {
  return await getHistoryGeneric(league, ItemOption.UniqueArmour, id);
};
