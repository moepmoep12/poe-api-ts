...
import { ScarabOverview } from "./ItemOverview";
...
): Promise<ScarabOverview> => {
  return await getOverviewGeneric(league, ItemOption.Scarab, language, ScarabOverview);
};
...
export const getHistory = async (league: string, id: number): Promise<HistoryPoint[]> => {
  return await getHistoryGeneric(league, ItemOption.Scarab, id);
};
