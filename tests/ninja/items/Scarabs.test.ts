...
import { ScarabOverview } from "../../../src/ninja/apis/items/scarabs";

if (process.env.MOCHA_WORKER_ID) mochaGlobalSetup();

describe(`PoE.Ninja - Items - Scarabs`, function () {
  this.timeout(5000);

  const league = "Standard";
  let result: ScarabOverview;

  it(`#getOverview(${league}) - should return ItemOverview`, async () => {
    result = <ScarabOverview>await expect(Items.Scarabs.getOverview(league)).to.be.fulfilled;
  });
