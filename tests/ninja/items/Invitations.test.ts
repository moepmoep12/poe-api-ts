...
import { InvitationOverview } from "../../../src/ninja/apis/items/invitations";

if (process.env.MOCHA_WORKER_ID) mochaGlobalSetup();

describe(`PoE.Ninja - Items - Invitations`, function () {
  this.timeout(5000);

  const league = "Standard";
  let result: InvitationOverview;

  it(`#getOverview(${league}) - should return ItemOverview`, async () => {
    result = <InvitationOverview>(
      await expect(Items.Invitations.getOverview(league)).to.be.fulfilled
    );
  });
